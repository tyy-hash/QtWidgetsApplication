...
cv::Mat convertQImageToCvMat(const QImage& qImage) {
    ...
    // 将RGB转为OpenCV默认的BGR顺序
    cv::Mat cvMat;
    cv::cvtColor(tmp, cvMat, cv::COLOR_RGB2BGR);

    // 克隆数据以确保独立于QImage的生命周期
    return cvMat.clone();
}
...
{
    bool runOnGPU = false;
    TCT inf("D:/vs_project/QtTCT/best.onnx", cv::Size(640, 640), "classes.txt", runOnGPU); //处理模型

    //std::vector<std::string> imageNames;
    //imageNames.push_back("D:/4.png");           //读取图片
